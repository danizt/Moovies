...
public class TaskDetallesEn extends AsyncTask<Void, Integer, Boolean> {
    private Context context;
    public ProgressDialog pDialog;
    private String id_detalle;

    public TaskDetallesEn(Context context, String id_detalle) {
    ...
    protected Boolean doInBackground(Void... voids) {
        ...
        try {
            if (Locale.getDefault().getLanguage().equals("ca")) {
                url = API_URL_GET_DETALLES.replace("<KEY>", API_KEY).replace("<ID>", id_detalle).replace("<LANG>", "es");
            } else {
                url = API_URL_GET_DETALLES.replace("<KEY>", API_KEY).replace("<ID>", id_detalle).replace("<LANG>", "en");
            }
